...
/* Live Drops: compact fixed feed on the home screen only. */
...
/* The modal keeps its controls in the old cost row position; the redundant cost label is removed. */
...
/* iPhone 13/13 Pro class widths: keep logo + online + profile on one line. */
header{min-width:0}
.logo{flex:0 1 auto;white-space:nowrap;min-width:0}
.top-right{min-width:0;flex:0 1 auto;white-space:nowrap}
.top-pill,.profile-top{white-space:nowrap}
...
function patchOnline(){const n=document.getElementById('onlineCount');if(!n||n.dataset.edOnlinePatched)return;const host=n.parentElement;if(!host)return;host.innerHTML=`<span class="online-dot" aria-hidden="true"></span><span id="onlineCount">${n.textContent||'128'}</span> online`;n.dataset.edOnlinePatched='1'}
...
window.__emojiDropsMotion={version:2,reduced:reduced(),visualOnly:true};
})();
