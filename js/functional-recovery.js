(() => {
  ...
  const updateRewardButtons = () => {
    ...
    document.querySelectorAll('#profilePage button').forEach(button => {
      if (text(button) === 'пополнить' || button.dataset.action === 'deposit') {
        button.textContent = available ? `Получить ${REWARD}₽` : `Получено · ${formatTimer(nextAt - Date.now())}`;
        button.disabled = !available;
        button.dataset.dailyReward = '1';
      }
  ...
  const addLiveDrop = (nickname, item) => {
    ...
    node.className = `ed-live-drop ed-live-${rarity}`;
    node.style.setProperty('--rarity-color', rarityColors[rarity] || rarityColors.common);
    ...
    while (container.children.length > 7) container.lastElementChild?.remove();
  ...
  const saveBest = item => {
    ...
    localStorage.setItem(BEST_KEY, JSON.stringify({ emoji:item.emoji, rarity:item.rarity, price:item.price, updatedAt:Date.now() }));
    try { window.updateBestDrop?.(); } catch (_) {}
  };
  ...
  const bridgeBestDrop = () => {
    ...
    const wrapped = function(...args) {
      const wins = Array.isArray(window.state?.winQueue) ? window.state.winQueue : [];
      wins.forEach(saveBest);
  ...
  const syncBestDrop = () => {
    const best = readBest();
    if (!best) return;
    document.querySelectorAll('#bestDrop,.best-drop,.best-drop-card').forEach(node => {
      const emoji = node.querySelector('.best-drop-emoji,.drop-emoji');
  ...
  const markPrimaryButtons = () => {
    document.querySelectorAll('button').forEach(button => {
      const value = text(button);
      if (value.includes('апгрейд') || value.includes('статистика') || value.includes('настройки') || value.includes('пополнить') || value.includes('выйти')) button.classList.add('ed-primary-action');
    });
  };
  ...
  const routeSemanticButtons = event => {
    ...
    if (button.dataset.dailyReward === '1' || value.startsWith('получить ') || value.startsWith('получено ·')) {
      event.preventDefault(); event.stopImmediatePropagation(); claimReward(); return;
    }
  ...
  const ensureViewVisibility = () => {
    const views = [$('settingsOverlay'), $('statsOverlay'), $('edUpgrade2'), $('upgradePage')].filter(Boolean);
    views.forEach(view => {
      if (view.classList.contains('open')) { view.hidden = false; view.style.display = 'flex'; }
    });
  };
  ...
  const install = () => {
    ...
    document.querySelectorAll('.balance-dropdown button').forEach(button => button.dataset.dailyReward = '1');
    setInterval(() => { updateRewardButtons(); syncBestDrop(); bridgeBestDrop(); }, 1000);
  };
