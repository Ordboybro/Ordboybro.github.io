...
function arm(){if(armed)return;armed=true;document.addEventListener('click',e=>{if(e.target.closest?.('[data-view="market"]'))clean()},true);let timer=0;const mo=new MutationObserver(()=>{if(timer)return;timer=setTimeout(()=>{timer=0;const r=root();if(!r||!r.classList.contains('active'))return;const owner=r.querySelector(':scope > .v26-market');if(owner)[...r.children].forEach(x=>{if(x!==owner)x.remove()});else if(r.querySelector('[data-buy],.ed-market-card,.ed-shared-market-head'))r.replaceChildren()},100)});mo.observe(document.body,{childList:true,subtree:true});clean()}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',arm,{once:true});else arm();
window.__emojiDropsMarketAuthorityFinal={version:2,observerDebounceMs:100};
})();
