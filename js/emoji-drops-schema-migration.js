...
function migrate(){const current=read(KEY);if(current&&Number(current.schemaVersion||0)>=VERSION){window.__emojiDropsSchema={version:VERSION,migrated:false};return}
 const cloudConfigured=!!(window.EMOJI_DROPS_SUPABASE?.url&&window.EMOJI_DROPS_SUPABASE?.publishableKey);const legacy=read('users'),userEmail=localStorage.getItem('currentUser');let source=current;
 if(!source&&!cloudConfigured&&Array.isArray(legacy)&&userEmail){const u=legacy.find(x=>x&&x.email===userEmail);if(u)source={...u,stats:u.stats||{}}}
