...
let timer=0,interval=0,observer=0,channel=0,realtimeReady=false,fallbackTimer=0,retryTimer=0,lastKey='',lastPaintState='',inFlight=false,booted=false,destroyed=false,generation=0,authReady=0,authChange=0,visibilityHandler=0,pagehideHandler=0,pageshowHandler=0,lastRows=[];
...
function paint(rows,configured,error,status){
 ...
 const clean=Array.isArray(rows)?rows.slice(0,20):[];const previousKeys=new Set(lastRows.map(x=>[x?.created_at,x?.nickname,x?.case_id,x?.item_price,x?.item?.emoji,x?.item?.rarity].join('|')));lastRows=clean;
 ...
 const cards=clean.map(function(d){
   const i=d&&d.item||{};
   const rarity=String(i.rarity||'common').toLowerCase();const rowKey=[d?.created_at,d?.nickname,d?.case_id,d?.item_price,i?.emoji,i?.rarity].join('|');const fresh=previousKeys.has(rowKey)?'':' is-new';return '<article class="ed-live-card'+fresh+'" data-rarity="'+esc(rarity)+'"><div class="e">'+esc(i.emoji||'🎁')+'</div><div><b>'+esc(d.nickname||'Player')+' · '+esc(rarity.toUpperCase())+'</b><span>'+esc(d.case_id||i.case_id||'Case')+' · '+(d.created_at?new Date(d.created_at).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'}):'')+'</span><strong>'+rub(d.item_price??i.price)+'</strong></div></article>'
 }).join('');
...
function scheduleRetry(){if(destroyed||retryTimer||document.hidden)return;retryTimer=window.setTimeout(()=>{retryTimer=0;if(booted&&!channel&&!destroyed&&!document.hidden)subscribe()},1200)}
function subscribe(){const c=client();if(!c?.channel||channel||destroyed)return;const token=++generation;try{realtimeReady=false;if(fallbackTimer){clearTimeout(fallbackTimer);fallbackTimer=0}channel=c.channel('emoji-drops-live-final').on('broadcast',{event:'live_drop'},payload=>{if(destroyed||token!==generation||!channel)return;const d=payload?.payload;if(d&&typeof d==='object'){const row={nickname:d.nickname,item:d.item&&typeof d.item==='object'?{emoji:d.item.emoji,rarity:d.item.rarity,price:d.item.price}:null,case_id:d.case_id,item_price:d.item_price,created_at:d.created_at};lastRows=[row,...lastRows.filter(x=>!(x?.created_at===row.created_at&&x?.nickname===row.nickname&&x?.case_id===row.case_id&&x?.item_price===row.item_price))].slice(0,20);lastPaintState='SUBSCRIBED';paint(lastRows,true,false,'SUBSCRIBED')}else schedule()}).subscribe(status=>{if(destroyed||token!==generation||!channel)return;if(status==='SUBSCRIBED'){realtimeReady=true;if(retryTimer){clearTimeout(retryTimer);retryTimer=0}if(fallbackTimer){clearTimeout(fallbackTimer);fallbackTimer=0}if(interval){clearInterval(interval);interval=0}paint(lastRows,true,false,'SUBSCRIBED')}else if(status==='CHANNEL_ERROR'||status==='TIMED_OUT'||status==='CLOSED'){realtimeReady=false;const ch=channel;channel=0;try{const removed=c.removeChannel?.(ch);if(!removed)ch.unsubscribe?.()}catch{try{ch.unsubscribe?.()}catch{}}if(booted&&!document.hidden&&!interval)interval=window.setInterval(refresh,3000);paint(lastRows,true,false,interval?'FALLBACK':'CONNECTING');scheduleRetry()}})}catch{channel=0;realtimeReady=false;if(booted&&!document.hidden&&!interval)interval=window.setInterval(refresh,3000);paint(lastRows,true,false,'FALLBACK');scheduleRetry()}}
function stopLoops(unsubscribe=false){if(interval){clearInterval(interval);interval=0}if(unsubscribe&&retryTimer){clearTimeout(retryTimer);retryTimer=0}if(unsubscribe&&channel){const ch=channel;channel=0;realtimeReady=false;++generation;if(fallbackTimer){clearTimeout(fallbackTimer);fallbackTimer=0}try{const client=window.EmojiDropsAuth?.client;const removed=client?.removeChannel?.(ch);if(!removed)ch.unsubscribe?.()}catch{try{ch.unsubscribe?.()}catch{}}}}
function startLoops(){if(!booted||destroyed||document.hidden)return;if(!realtimeReady&&!interval)interval=window.setInterval(refresh,3000)}
function bindInteractionPause(){const root=document.getElementById(ID);if(root)root.dataset.lifecycleBound='1'}
function boot(){
 ...
 authReady=()=>{schedule();subscribe()};authChange=()=>{if(channel){const ch=channel;channel=0;realtimeReady=false;++generation;try{const removed=window.EmojiDropsAuth?.client?.removeChannel?.(ch);if(!removed)ch.unsubscribe?.()}catch{try{ch.unsubscribe?.()}catch{}}}if(interval){clearInterval(interval);interval=0}if(fallbackTimer){clearTimeout(fallbackTimer);fallbackTimer=0}schedule();subscribe();fallbackTimer=window.setTimeout(()=>{if(booted&&!realtimeReady&&!document.hidden&&!interval)interval=window.setInterval(refresh,3000);if(booted&&!realtimeReady)paint(lastRows,true,false,'FALLBACK')},6500)};window.addEventListener('emoji-drops-auth-ready',authReady,{passive:true});
