...
let timer=0,interval=0,rotateTimer=0,observer=0,channel=0,realtimeReady=false,fallbackTimer=0,lastKey='',lastPaintState='',inFlight=false,booted=false,authReady=0,authChange=0,visibilityHandler=0,pagehideHandler=0,pageshowHandler=0,lastRows=[],rotationPaused=false;
...
function css(){
 ...
 s.textContent='#edRealLiveDrops{margin:14px 0 18px;padding:14px;border:1px solid #302a23;border-radius:20px;background:radial-gradient(circle at 0 0,#ff8a0012,transparent 38%),linear-gradient(145deg,#151515,#0a0a0a);box-shadow:inset 0 1px #fff1,0 14px 34px #0007}#edRealLiveDrops .ed-live-head{display:flex;justify-content:space-between;align-items:end;gap:10px;margin-bottom:10px}#edRealLiveDrops h2{font-size:16px;margin:0}#edRealLiveDrops .ed-live-status{font-size:9px;color:#8ddf9a;font-weight:900;letter-spacing:.08em}#edRealLiveDrops .ed-live-list{display:flex;gap:9px;overflow-x:auto;scrollbar-width:none;padding:2px 1px 4px}#edRealLiveDrops .ed-live-list::-webkit-scrollbar{display:none}#edRealLiveDrops .ed-live-card{--rarity:#9ca3af;flex:0 0 190px;min-width:0;display:grid;grid-template-columns:44px 1fr;gap:9px;align-items:center;padding:9px;border:1px solid #292929;border-radius:14px;background:linear-gradient(145deg,#151515,#101010);border-color:color-mix(in srgb,var(--rarity) 55%,#292929)}#edRealLiveDrops .ed-live-card .e{width:44px;height:44px;display:grid;place-items:center;border-radius:11px;background:#0b0b0b;font-size:25px}#edRealLiveDrops .ed-live-card b{display:block;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}#edRealLiveDrops .ed-live-card span{display:block;color:#888;font-size:9px;margin-top:3px}#edRealLiveDrops .ed-live-card strong{display:block;color:#ffbd62;font-size:10px;margin-top:3px}#edRealLiveDrops .ed-live-card:hover{transform:translateY(-3px);box-shadow:0 12px 28px #000a,0 0 20px #ff8a0010}#edRealLiveDrops .ed-live-card.common{--live-rarity:#9ca3af}.ed-live-card.rare{--live-rarity:#60a5fa}.ed-live-card.epic{--live-rarity:#a78bfa}.ed-live-card.mythical{--live-rarity:#e879f9}.ed-live-card.legendary{--live-rarity:#ff8a18}#edRealLiveDrops .ed-live-empty{padding:15px 12px;text-align:center;color:#999;border:1px dashed #39332b;border-radius:14px;min-width:100%;min-height:74px;display:grid;place-items:center;align-content:center;gap:2px;background:linear-gradient(145deg,#111,#0b0b0b)}#edRealLiveDrops .ed-live-empty b{display:block;color:#ddd;margin-bottom:5px}[data-rarity=common]{--rarity:#9ca3af}[data-rarity=rare]{--rarity:#60a5fa}[data-rarity=epic]{--rarity:#a78bfa}[data-rarity=mythical]{--rarity:#e879f9}[data-rarity=legendary]{--rarity:#ff8a18;box-shadow:0 0 0 1px #ff8a0030,0 8px 26px #0008}#edRealLiveDrops{margin-top:14px;padding:10px;border-radius:17px}#edRealLiveDrops .ed-live-card{flex-basis:170px}@media(prefers-reduced-motion:reduce){#edRealLiveDrops *{animation:none!important;transition:none!important}}';
...
function subscribe(){const c=client();if(!c?.channel||channel)return;try{realtimeReady=false;if(fallbackTimer){clearTimeout(fallbackTimer);fallbackTimer=0}channel=c.channel('emoji-drops-live-final').on('postgres_changes',{event:'INSERT',schema:'public',table:'live_drops'} ,()=>schedule()).subscribe(status=>{if(status==='SUBSCRIBED'){realtimeReady=true;if(fallbackTimer){clearTimeout(fallbackTimer);fallbackTimer=0}if(interval){clearInterval(interval);interval=0}paint(lastRows,true,false,'SUBSCRIBED')}else if(status==='CHANNEL_ERROR'||status==='TIMED_OUT'||status==='CLOSED'){realtimeReady=false;if(booted&&!document.hidden&&!interval)interval=window.setInterval(refresh,3000);paint(lastRows,true,false,interval?'FALLBACK':'CONNECTING')}})}catch{channel=0;realtimeReady=false;if(booted&&!document.hidden&&!interval)interval=window.setInterval(refresh,3000);paint(lastRows,true,false,'FALLBACK')}}
function stopLoops(unsubscribe=false){if(interval){clearInterval(interval);interval=0}if(rotateTimer){clearInterval(rotateTimer);rotateTimer=0}if(unsubscribe&&channel){try{channel.unsubscribe?.()}catch{}channel=0;realtimeReady=false;if(fallbackTimer){clearTimeout(fallbackTimer);fallbackTimer=0}}}
