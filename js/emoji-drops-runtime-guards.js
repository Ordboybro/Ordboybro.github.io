(()=>{'use strict';
/* Emoji Drops runtime guards v1. Post-core safety layer. No visual redesign. */
...
function repair(){const s=state();if(!s)return false;let changed=false;
  if(!Number.isFinite(Number(s.balance))||s.balance<0){s.balance=0;changed=true}
  if(!Array.isArray(s.inventory)){s.inventory=[];changed=true}
  if(!Array.isArray(s.market)){s.market=[];changed=true}
  s.stats=s.stats&&typeof s.stats==='object'?s.stats:{};
  for(const k of ['opens','wins','spent','earned','upgrades']){if(!Number.isFinite(Number(s.stats[k]))||s.stats[k]<0){s.stats[k]=0;changed=true}}
  if(!Number.isFinite(Number(s.xp))||s.xp<0){s.xp=0;changed=true}
  if(!Number.isFinite(Number(s.level))||s.level<1){s.level=1;changed=true}
...
function updateRewardLabel(){const b=document.querySelector('[data-reward]');if(!b)return;const left=dueMs();if(left<=1000){b.textContent='+250 ₽';return}const m=Math.floor(left/60000),sec=Math.floor((left%60000)/1000);b.textContent=`Бонус через ${m}:${String(sec).padStart(2,'0')}`}
