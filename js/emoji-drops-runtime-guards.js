(()=>{'use strict';
/* Emoji Drops runtime guards v6. Emergency bonus only when balance cannot afford the cheapest case. Max 3 claims per local day. No visual redesign. */
const KEY='emojiDropsStateV3',MAX_DAILY=3,BONUS=250,BLOCK_MS=100*365*24*60*60*1000,MAX_SAFE=Number.MAX_SAFE_INTEGER;
...
if(!Array.isArray(s.inventory)){s.inventory=[];changed=true}else{const seen=new Set();const next=[];for(const x of s.inventory){const y=cleanItem(x);if(!y)continue;if(seen.has(y.id))y.id=`${y.id}:${next.length}`;seen.add(y.id);next.push(y)}if(next.length!==s.inventory.length||next.some((x,i)=>x!==s.inventory[i])){s.inventory=next;changed=true}}
if(!Array.isArray(s.market)){s.market=[];changed=true}else{const next=s.market.map((x,i)=>{const y=cleanItem(x,`market:${i}`);if(y)y.seller=String(y.seller||'Player').trim().slice(0,32)||'Player';return y}).filter(Boolean);if(next.length!==s.market.length)s.market=next,changed=true;else if(next.some((x,i)=>JSON.stringify(x)!==JSON.stringify(s.market[i])))s.market=next,changed=true}
...
window.__emojiDropsRuntimeGuards={version:2,repair,claimDue,eligible};
})();
