(()=>{'use strict';
/* Emoji Drops runtime guards v5. Emergency bonus only when balance cannot afford the cheapest case. Max 3 claims per local day. No visual redesign. */
const KEY='emojiDropsStateV3',MAX_DAILY=3,BONUS=250,BLOCK_MS=100*365*24*60*60*1000;
...
function claimDue(){repair();const s=state();if(!s)return false;if(Number(s.balance)>=cheapest()){toast(`Сначала потрать деньги: минимум ${money(cheapest())}`);updateRewardLabel();return true}if(Number(s.bonusClaims||0)>=MAX_DAILY){toast(`Лимит бонусов на сегодня: ${MAX_DAILY}/${MAX_DAILY}`);updateRewardLabel();return true}s.balance=(Number(s.balance)||0)+BONUS;s.stats=s.stats||{};s.stats.earned=(Number(s.stats.earned)||0)+BONUS;s.xp=(Number(s.xp)||0)+10;s.bonusClaims=(Number(s.bonusClaims)||0)+1;s.bonusDay=dayKey();s.lastReward=Date.now()+BLOCK_MS;persist(s);core()?.render?.();toast(`Получено ${money(BONUS)}`);updateRewardLabel();return true}
