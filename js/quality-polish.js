(() => {
  ...
  function injectStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      html { scroll-behavior: smooth; scrollbar-color: #303030 #0b0b0b; scrollbar-width: thin; }
      body { overflow-x: hidden !important; }
      button, a, input, select, textarea { -webkit-tap-highlight-color: transparent; }
      button, .top-btn, .amount-btn, .case-card, .case, .sell-item-btn { transition: transform .18s ease, opacity .18s ease, box-shadow .18s ease, border-color .18s ease, background-color .18s ease; }
      ...
      .case-price { position: relative !important; left: auto !important; bottom: auto !important; margin-top: 10px !important; font-size: 17px !important; line-height: 1.1 !important; padding: 7px 12px !important; display: inline-flex !important; align-items: center; justify-content: center; width: max-content; max-width: calc(100% - 20px); z-index: 3; }
      ...
      @media (max-width: 900px) {
        ...
        .live-drops, #liveDrops, #liveContainer, #liveDropsContainer, .live-drops-container { min-height: 74px; padding-inline: 7px !important; gap: 7px; }
      }
  ...
  function decorateHandlers() {
    ...
    const decorate = (name, selector, duration) => {
      ...
      window[name] = function polishedHandler(...args) {
        ...
        Promise.resolve(result).then(() => { const element = document.querySelector(selector); if (element && getComputedStyle(element).display !== "none") animateIn(element, { duration }); });
        return result;
      };
    };
    ...
    if (typeof originalStartUpgrade === "function") {
      window.startUpgrade = function polishedUpgrade(...args) { const result = originalStartUpgrade.apply(this, args); syncBalance(); return result; };
    }
