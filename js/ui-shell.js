(() => {
    ...
    const ensure = (id, html) => {
        ...
        <div class="profile-main">
            ...
            <div class="profile-side"><button class="profile-mini-btn" onclick="logout()">🚪 Выйти</button></div>
        </div>
    ...
    ensure("settingsOverlay", `<div class="settings-overlay" id="settingsOverlay" style="display:none"><div class="settings-box"><div class="settings-line"></div><div class="settings-header"><div class="settings-title">Настройки</div><button class="settings-close" onclick="closeSettings()">✕</button></div><div class="settings-list"><div class="settings-item-column"><div class="settings-name">Новый ник</div><input class="settings-input" id="newNickname" placeholder="Ник"><button class="settings-action-btn" onclick="changeNickname()">Сохранить</button></div><div class="settings-item-column"><div class="settings-name">Смена пароля</div><input class="settings-input" id="oldPassword" type="password" placeholder="Старый пароль"><input class="settings-input" id="newPasswordSettings" type="password" placeholder="Новый пароль"><button class="settings-action-btn" onclick="changePassword()">Изменить пароль</button></div><div class="settings-item"><div><div class="settings-name red">Удалить аккаунт</div><div class="settings-sub">Действие нельзя отменить</div></div><button class="settings-action-btn delete-btn" onclick="deleteAccount()">Удалить</button></div></div></div></div>`);
