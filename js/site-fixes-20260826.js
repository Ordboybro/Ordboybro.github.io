...
(() => {
  ...
  function patchLiveDrop(){const original=window.addLiveDrop;if(typeof original!=='function'||original.__edOwnLivePatch)return false;const wrapped=function(username,item){original.apply(this,arguments);const c=document.getElementById('liveContainer');const card=c?.firstElementChild;if(card&&state()?.currentUser?.nickname===username)card.classList.add('live-drop','own',item?.rarity||'common');};wrapped.__edOwnLivePatch=true;window.addLiveDrop=wrapped;return true;}
