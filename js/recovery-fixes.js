(() => {
  ...
  function buildTrack(track, items, winner, winnerIndex = 30) {
    ...
    for (let index = 0; index < 42; index += 1) {
      const item = index === winnerIndex ? winner : (window.getRandomByChance?.(safeItems) || safeItems[index % safeItems.length]);
  ...
  function animateTrack(roulette, delay = 0) {
    ...
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        track.style.transform = `translate3d(${targetX - 18}px,0,0)`;
        setTimeout(() => {
  ...
  function persistAfterOpen(wins, price) {
    ...
    if (Number(state.balance) < price) return false;
    ...
    try {
      ...
      if (fast) {
        roulettes.forEach(roulette => {
          ...
          const x = rr.width / 2 - (wr.left - rr.left) - wr.width / 2;
      ...
      if (!persistAfterOpen(wins, totalPrice)) throw new Error('Opening commit failed');
