...
const run=()=>{
  ...
  pass('cross-tab Web Locks protection',s?.webLocks===true,s?.mode||'fallback');
  ...
  const failed=checks.filter(x=>!x.ok);window.__emojiDropsFinalQA={ok:failed.length===0,checks,failedCount:failed.length,ranAt:new Date().toISOString()};
