...
function hideAll(){const exact=document.getElementById('edExact');if(exact){exact.classList.remove('show');exact.setAttribute('aria-hidden','true');exact.style.display='none'}document.querySelectorAll('.ed-modal.show').forEach(m=>{m.classList.remove('show');m.style.display='none'});document.body.classList.remove('modal-lock')}
