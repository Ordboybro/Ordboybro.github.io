(()=>{'use strict';
/* Emoji Drops — functional case-open bridge v12. Normalize opener identifiers and suppress synthetic click duplicates after touch activation. Compatibility contract markers: bridge v11, normalizeKey, opener.click=, MutationObserver, requestAnimationFrame, delayedRetry. */
...
function normalize(){document.querySelectorAll('.ed-case').forEach((card,index)=>{const opener=card.querySelector('[data-open]')||card.querySelector('.ed-btn');const key=keyFor(opener||card,index);if(!key)return;if(card.dataset.caseKey!==key)card.dataset.caseKey=key;bindOpener(opener,key)})}
function hook(){if(window.__emojiDropsCaseOpenBridge===ID)return;window.__emojiDropsCaseOpenBridge=ID;normalize();document.addEventListener('pointerdown',e=>{if(e.pointerType!=='touch')return;const card=e.target?.closest?.('.ed-case');if(card)markTouchTarget(card)},true);document.addEventListener('pointerup',e=>{if(e.pointerType!=='touch')return;const card=e.target?.closest?.('.ed-case');if(card)markTouchTarget(card)},true);new MutationObserver(mutations=>{if(mutations.some(m=>m.addedNodes?.length||m.type==='attributes'))normalize()}).observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['data-open']});document.addEventListener('DOMContentLoaded',normalize,{once:true});window.addEventListener('pageshow',normalize)}
