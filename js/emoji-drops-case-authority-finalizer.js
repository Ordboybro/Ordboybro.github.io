...
document.addEventListener('click',e=>{const open=e.target?.closest?.('.edx-open button');if(open){e.preventDefault();e.stopImmediatePropagation();queueMicrotask(()=>open.onclick?.call(open));return}if(e.target?.closest?.('.edx-close,[aria-label="Close"]'))closeExact()},{capture:true});
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&document.querySelector('#edExact.show,#edV24Case.show,#edV24Reward.show,[role="dialog"].show:not(#edExact)'))closeExact()},{capture:true});
const mo=new MutationObserver(()=>{const closing=Date.now()<Number(window.__emojiDropsCaseClosingUntil||0);const exact=document.getElementById('edExact');if(closing&&exact?.classList.contains('show')){exact.classList.remove('show');exact.setAttribute('aria-hidden','true')}if(closing||exact?.classList.contains('show'))hideLegacy()});
