...
document.addEventListener('click',e=>{if(e.target?.closest?.('.edx-close')){closeExact()}},{capture:true});
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&document.getElementById('edExact')?.classList.contains('show'))closeExact()},{capture:true});
