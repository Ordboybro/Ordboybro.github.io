(()=>{'use strict';
/* Emoji Drops transaction layer v3. Journal-first atomicity, verification, recovery and Web Locks with lease fallback. */
const KEY='emojiDropsStateV3',JOURNAL='emojiDropsTxnV3',LOCK='emojiDropsLockV3',MAX_HISTORY=200,LOCK_MS=1800;
...
let active=null,owner=`${now()}-${Math.random().toString(36).slice(2)}`;
...
function begin(label){const s=core()?.state?.();if(!s||active)return null;const before=clone(s),raw=get(KEY),id=`${now()}-${Math.random().toString(36).slice(2)}`;if(!before)return null;const j={v:3,id,label:String(label||'action'),before,raw,beforeHash:digest(raw||''),startedAt:now(),status:'pending'};if(!journalWrite(j))return null;active={...j};return active}
function commit(t){if(!t||active!==t)return false;const s=core()?.state?.();if(!s){rollback(t);return false}const raw=get(KEY);let parsed=null;try{parsed=JSON.parse(raw||'null')}catch{}if(!parsed||digest(raw)!==digest(JSON.stringify(s))){rollback(t);return false}const after=clone(s),afterHash=digest(raw);t.status='committed';t.after=after;t.afterHash=afterHash;t.finishedAt=now();if(!journalWrite(t)){rollback(t);return false}active=null;return true}
function rollback(t){if(!t)return false;const ok=restore(t);journalWrite({v:3,id:t.id,label:t.label,before:clone(t.before),raw:t.raw,beforeHash:t.beforeHash,startedAt:t.startedAt,finishedAt:now(),status:ok?'rolled_back':'rollback_failed'});if(active===t)active=null;return ok}
...
function acquire(){const old=readLease(),t=now();if(old&&Number(old.expires)>t&&old.owner!==owner)return false;return set(LOCK,JSON.stringify({owner,expires:t+LOCK_MS}))&&readLease()?.owner===owner}
function refresh(){const x=readLease();if(x?.owner===owner)set(LOCK,JSON.stringify({owner,expires:now()+LOCK_MS}))}
function release(){if(readLease()?.owner===owner)remove(LOCK)}
function withLock(fn){if(navigator?.locks?.request)return navigator.locks.request('emoji-drops-state',async()=>fn());if(!acquire())return Promise.reject(new Error('transaction_locked'));try{return Promise.resolve(fn())}finally{release()}}
...
window.__emojiDropsTransactions={version:3,begin,commit,rollback,recover,acquire,refresh,release,withLock,history,record,maxHistory:MAX_HISTORY,journal:JOURNAL,lock:LOCK};
})();
