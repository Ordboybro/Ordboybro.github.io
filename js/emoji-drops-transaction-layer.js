...
const KEY='emojiDropsStateV3',JOURNAL='emojiDropsTxnV4',LOCK='emojiDropsLockV4',MAX_HISTORY=200,LOCK_MS=1800,COMMIT_GRACE_MS=1200;
const stateKey=()=>core()?.storageKey?.()||KEY;
const journalKey=()=>`${JOURNAL}:${window.EmojiDropsAuth?.userId||'guest'}`;
const lockKey=()=>`${LOCK}:${window.EmojiDropsAuth?.userId||'guest'}`;const PHASES=Object.freeze({PREPARING:'PREPARING',SUBMITTING:'SUBMITTING',COMMITTED:'COMMITTED',ANIMATING:'ANIMATING',REVEALED:'REVEALED',ROLLED_BACK:'ROLLED_BACK'});
...
const set=(k,v)=>{if(fault('storageWrite')){if(k===stateKey()&&active&&!rollingBack){rollingBack=true;try{rollback(active)}finally{rollingBack=false}}throw new Error('fault_storage_write')}try{localStorage.setItem(k,v);return true}catch{return false}};
const remove=k=>{if(fault('storageWrite')){if(k===stateKey()&&active&&!rollingBack){rollingBack=true;try{rollback(active)}finally{rollingBack=false}}throw new Error('fault_storage_write')}try{localStorage.removeItem(k);return true}catch{return false}};
const now=()=>Date.now();
...
function journalWrite(x){return set(journalKey(),JSON.stringify(x))}
function journalRead(){try{const x=JSON.parse(get(journalKey())||'null');return x&&typeof x==='object'?x:null}catch{return null}}
function syncPersisted(){const raw=get(stateKey()),s=core()?.state?.();if(!s||!raw)return false;try{const incoming=JSON.parse(raw);for(const k of Object.keys(s))delete s[k];Object.assign(s,clone(incoming));core()?.render?.();return true}catch{return false}}
function restore(t){const s=core()?.state?.();if(!s||!t?.before)return false;try{for(const k of Object.keys(s))delete s[k];Object.assign(s,clone(t.before));if(t.raw==null)remove(stateKey());else if(!set(stateKey(),t.raw))return false;core()?.render?.();return true}catch{return false}}
function begin(label){if(fault('transaction'))return null;const s=core()?.state?.();if(!s||active)return null;const before=clone(s),raw=get(stateKey()),id=`${now()}-${Math.random().toString(36).slice(2)}`;if(!before)return null;const j={v:11,id,label:String(label||'action'),phase:PHASES.PREPARING,before,raw,beforeHash:digest(raw||''),startedAt:now(),status:'pending'};if(!journalWrite(j))return null;active={...j};return active}
function readLease(){try{return JSON.parse(get(lockKey())||'null')}catch{return null}}
function leaseOwned(){return readLease()?.owner===owner}
function commit(t){if(!t||active!==t)return false;if(t.leaseOwner&&!leaseOwned()){syncPersisted();active=null;return false}const s=core()?.state?.();if(!s){rollback(t);return false}const raw=get(stateKey());let parsed=null;try{parsed=JSON.parse(raw||'null')}catch{}if(!parsed||digest(raw)!==digest(JSON.stringify(s))){rollback(t);return false}const after=clone(s);t.phase=PHASES.COMMITTED;t.status='committed';t.after=after;t.afterHash=digest(raw);t.finishedAt=now();if(!journalWrite(t)){rollback(t);return false}active=null;return true}
...
function recover(){const j=journalRead();if(!j||j.status!=='pending'||!j.before)return false;const current=get(stateKey()),currentHash=digest(current||'');if(j.afterHash&&currentHash===j.afterHash){journalWrite({...j,v:11,phase:PHASES.COMMITTED,status:'recovered_committed',recoveredAt:now()});return true}const s=core()?.state?.();try{if(s){for(const k of Object.keys(s))delete s[k];Object.assign(s,clone(j.before));if(j.raw==null)remove(stateKey());else if(!set(stateKey(),j.raw))throw Error('recovery_write_failed');core()?.render?.()}journalWrite({...j,v:11,phase:PHASES.ROLLED_BACK,status:'recovered_rolled_back',recoveredAt:now()});return true}catch{try{journalWrite({...j,v:11,status:'recovery_failed',recoveredAt:now()})}catch{}return false}}
function acquire(){const old=readLease(),t=now();if(old&&Number(old.expires)>t&&old.owner!==owner)return false;if(!set(lockKey(),JSON.stringify({owner,expires:t+LOCK_MS})))return false;const ok=leaseOwned();if(ok&&!lockTimer)lockTimer=setInterval(()=>{if(active){const x=readLease();if(x?.owner===owner)set(lockKey(),JSON.stringify({owner,expires:now()+LOCK_MS}))}},500);return ok}
function refresh(){if(leaseOwned())set(lockKey(),JSON.stringify({owner,expires:now()+LOCK_MS}))}
function release(){if(lockTimer){clearInterval(lockTimer);lockTimer=0}if(leaseOwned())remove(lockKey())}
...
async function run(label,executor,apply,options){
  if(typeof executor!=='function')throw new Error('transaction_executor_required');
  return withLock(async()=>{
    const remoteOperation=options?.remote!==false;
    const t=begin(label);
    ...
    try{
      ...
      remoteCommitted=remoteOperation;
...
function wrapStorage(){if(localStorage.__edTxnV4)return;const os=localStorage.setItem.bind(localStorage),orm=localStorage.removeItem?.bind(localStorage);try{localStorage.setItem=function(k,v){if(fault('storageWrite')){if(k===stateKey()&&active&&!rollingBack){rollingBack=true;try{rollback(active)}finally{rollingBack=false}}throw new Error('fault_storage_write')}try{return os(k,v)}catch(err){if(k===stateKey()&&active&&!rollingBack){rollingBack=true;try{rollback(active)}finally{rollingBack=false}}throw err}};if(orm)localStorage.removeItem=function(k){if(k===stateKey()&&active&&!rollingBack){rollingBack=true;try{rollback(active)}finally{rollingBack=false}}return orm(k)};Object.defineProperty(localStorage,'__edTxnV4',{value:true})}catch{}}
