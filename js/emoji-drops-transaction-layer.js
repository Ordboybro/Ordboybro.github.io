(()=>{'use strict';
/* Emoji Drops transaction layer v8. Web Locks primary, lease fallback with ownership verification, journal-first atomicity, recovery and fault injection. */
...
function rollback(t){if(!t)return false;if(t.leaseOwner&&!leaseOwned()){syncPersisted();if(active===t)active=null;return false}let ok=false;try{ok=restore(t)}catch{}try{journalWrite({v:8,id:t.id,label:t.label,before:clone(t.before),raw:t.raw,beforeHash:t.beforeHash,startedAt:t.startedAt,finishedAt:now(),status:ok?'rolled_back':'rollback_failed'})}catch{}if(active===t)active=null;return ok}
