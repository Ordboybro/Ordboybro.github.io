(()=>{'use strict';
/* Emoji Drops transaction layer v4. Journal-first atomicity, verification, recovery and Web Locks with renewable lease fallback. */
const KEY='emojiDropsStateV3',JOURNAL='emojiDropsTxnV4',LOCK='emojiDropsLockV4',MAX_HISTORY=200,LOCK_MS=1800;
...
let active=null,lockTimer=0,owner=`${now()}-${Math.random().toString(36).slice(2)}`;
...
function begin(label){const s=core()?.state?.();if(!s||active)return null;const before=clone(s),raw=get(KEY),id=`${now()}-${Math.random().toString(36).slice(2)}`;if(!before)return null;const j={v:4,id,label:String(label||'action'),before,raw,beforeHash:digest(raw||''),startedAt:now(),status:'pending'};if(!journalWrite(j))return null;active={...j};return active}
function commit(t){if(!t||active!==t)return false;const s=core()?.state?.();if(!s){rollback(t);return false}const raw=get(KEY);let parsed=null;try{parsed=JSON.parse(raw||'null')}catch{}if(!parsed||digest(raw)!==digest(JSON.stringify(s))){rollback(t);return false}const after=clone(s);t.status='committed';t.after=after;t.afterHash=digest(raw);t.finishedAt=now();if(!journalWrite(t)){rollback(t);return false}active=null;return true}
function rollback(t){if(!t)return false;const ok=restore(t);journalWrite({v:4,id:t.id,label:t.label,before:clone(t.before),raw:t.raw,beforeHash:t.beforeHash,startedAt:t.startedAt,finishedAt:now(),status:ok?'rolled_back':'rollback_failed'});if(active===t)active=null;return ok}
function recover(){const j=journalRead();if(!j||j.status!=='pending'||!j.before)return false;const current=get(KEY),currentHash=digest(current||'');if(j.afterHash&&currentHash===j.afterHash){journalWrite({...j,status:'recovered_committed',recoveredAt:now()});return true}const s=core()?.state?.();try{if(s){for(const k of Object.keys(s))delete s[k];Object.assign(s,clone(j.before));if(j.raw==null)remove(KEY);else set(KEY,j.raw);core()?.render?.()}journalWrite({...j,status:'recovered_rolled_back',recoveredAt:now()});return true}catch{journalWrite({...j,status:'recovery_failed',recoveredAt:now()});return false}}
function readLease(){try{return JSON.parse(get(LOCK)||'null')}catch{return null}}
function acquire(){const old=readLease(),t=now();if(old&&Number(old.expires)>t&&old.owner!==owner)return false;if(!set(LOCK,JSON.stringify({owner,expires:t+LOCK_MS})))return false;const ok=readLease()?.owner===owner;if(ok&&!lockTimer)lockTimer=setInterval(()=>{if(active){const x=readLease();if(x?.owner===owner)set(LOCK,JSON.stringify({owner,expires:now()+LOCK_MS}))}},500);return ok}
function refresh(){const x=readLease();if(x?.owner===owner)set(LOCK,JSON.stringify({owner,expires:now()+LOCK_MS}))}
function release(){if(lockTimer){clearInterval(lockTimer);lockTimer=0}if(readLease()?.owner===owner)remove(LOCK)}
function withLock(fn){const locks=typeof navigator!=='undefined'&&navigator?.locks?.request;if(locks)return navigator.locks.request('emoji-drops-state',async()=>fn());if(!acquire())return Promise.reject(new Error('transaction_locked'));try{return Promise.resolve(fn())}finally{release()}}
...
window.__emojiDropsTransactions={version:4,begin,commit,rollback,recover,acquire,refresh,release,withLock,history,record,maxHistory:MAX_HISTORY,journal:JOURNAL,lock:LOCK};
})();
