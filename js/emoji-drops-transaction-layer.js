...
function rollback(t){if(!t)return false;if(t.leaseOwner&&!leaseOwned()){syncPersisted();if(active===t)active=null;return false}let ok=false;try{ok=restore(t)}catch{}try{journalWrite({v:10,id:t.id,label:t.label,before:clone(t.before),raw:t.raw,beforeHash:t.beforeHash,startedAt:t.startedAt,finishedAt:now(),status:ok?'rolled_back':'rollback_failed'})}catch{}if(active===t)active=null;return ok}
function recover(){const j=journalRead();if(!j||j.status!=='pending'||!j.before)return false;const current=get(KEY),currentHash=digest(current||'');if(j.afterHash&&currentHash===j.afterHash){journalWrite({...j,v:10,status:'recovered_committed',recoveredAt:now()});return true}const s=core()?.state?.();try{if(s){for(const k of Object.keys(s))delete s[k];Object.assign(s,clone(j.before));if(j.raw==null)remove(KEY);else if(!set(KEY,j.raw))throw Error('recovery_write_failed');core()?.render?.()}journalWrite({...j,v:10,status:'recovered_rolled_back',recoveredAt:now()});return true}catch{try{journalWrite({...j,v:10,status:'recovery_failed',recoveredAt:now()})}catch{}return false}}
