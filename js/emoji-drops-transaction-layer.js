(()=>{'use strict';
/* Emoji Drops transaction layer v9. Web Locks primary, lease fallback with ownership verification, journal-first atomicity, recovery and fault injection. */
...
function rollback(t){if(!t)return false;if(t.leaseOwner&&!leaseOwned()){syncPersisted();if(active===t)active=null;return false}let ok=false;try{ok=restore(t)}catch{}try{journalWrite({v:9,id:t.id,label:t.label,before:clone(t.before),raw:t.raw,beforeHash:t.beforeHash,startedAt:t.startedAt,finishedAt:now(),status:ok?'rolled_back':'rollback_failed'})}catch{}if(active===t)active=null;return ok}
function recover(){const j=journalRead();if(!j||j.status!=='pending'||!j.before)return false;const current=get(KEY),currentHash=digest(current||'');if(j.afterHash&&currentHash===j.afterHash){journalWrite({...j,status:'recovered_committed',recoveredAt:now()});return true}const s=core()?.state?.();try{if(s){for(const k of Object.keys(s))delete s[k];Object.assign(s,clone(j.before));if(j.raw==null)remove(KEY);else if(!set(KEY,j.raw))throw Error('recovery_write_failed');core()?.render?.()}journalWrite({...j,status:'recovered_rolled_back',recoveredAt:now()});return true}catch{try{journalWrite({...j,status:'recovery_failed',recoveredAt:now()})}catch{}return false}}
...
function install(){document.addEventListener?.('click',e=>{const b=e.target?.closest?.('[data-do-open],[data-sell],[data-sell-all],[data-upgrade],[data-buy],[data-list-random],[data-daily],[data-reset]');if(!b||b.matches?.('#edExactBox .edx-open button,.ed-case [data-do-open]'))return;const keys=['doOpen','sell','sellAll','upgrade','buy','listRandom','daily','reset'];const label=keys.find(k=>b.dataset&&b.dataset[k]!==undefined)||'action';withLock(()=>new Promise(resolve=>{const t=begin(label);if(!t){resolve(false);return}if(!(typeof navigator!=='undefined'&&navigator?.locks?.request))t.leaseOwner=true;const before=clone(t.before);setTimeout(()=>{if(active===t){const after=clone(core()?.state?.());if(after)record(label,before,after);if(active===t)commit(t)}resolve(true)},COMMIT_GRACE_MS)})).catch(()=>{})},true)}
