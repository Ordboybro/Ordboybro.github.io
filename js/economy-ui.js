/* EmojiDrops economy presentation layer. Uses the calibrated odds as the single source of truth. */
(() => {
  'use strict';
  ...
  window.startUpgrade = function startUpgrade(sourceValue, targetValue, item) {
    ...
    const chance = window.EMOJI_DROPS_ECONOMY?.getUpgradeChance?.(source, target) ?? Math.max(1, Math.min(95, source / Math.max(target, 1) * 96));
    ...
    if (text) text.textContent = success ? `АПГРЕЙД УСПЕШЕН · ${chance.toFixed(1)}%` : `НЕ УДАЛОСЬ · ${chance.toFixed(1)}%`;
