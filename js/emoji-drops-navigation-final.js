...
function handle(e){const b=e.target?.closest?.('[data-view]'),v=b?.getAttribute('data-view');if(!b||!V.includes(v))return;const now=Date.now();if(e.type==='pointerdown'&&e.pointerType==='touch'){lastTouch=now;lastTouchTarget=b;commit(v);setTimeout(()=>{if(lastTouchTarget===b&&Date.now()-lastTouch>=700)lastTouchTarget=null},720);return}if(e.type==='click'){if(e.pointerType==='touch'){const validTouch=!!lastTouchTarget&&b===lastTouchTarget&&now-lastTouch<700;if(!validTouch){e.preventDefault();e.stopImmediatePropagation();return}lastTouchTarget=null;return}commit(v)}}
