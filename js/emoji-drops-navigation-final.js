...
const TOUCH_CLICK_GUARD_MS=1800;
...
function handle(e){const b=e.target?.closest?.('[data-view]'),v=b?.getAttribute('data-view');if(!b||!V.includes(v))return;const now=Date.now();if(e.type==='pointerdown'&&e.pointerType==='touch'){lastTouch=now;lastTouchTarget=b;commit(v);setTimeout(()=>{if(lastTouchTarget===b&&Date.now()-lastTouch>=TOUCH_CLICK_GUARD_MS)lastTouchTarget=null},TOUCH_CLICK_GUARD_MS+20);return}if(e.type==='click'){const recentTouch=now-lastTouch<TOUCH_CLICK_GUARD_MS;const validTouch=!!lastTouchTarget&&b===lastTouchTarget&&recentTouch;if(recentTouch){if(!validTouch){e.preventDefault();e.stopImmediatePropagation();return}lastTouchTarget=null;return}if(e.pointerType==='touch'){e.preventDefault();e.stopImmediatePropagation();return}commit(v)}}
...
window.__emojiDropsNavigationFinal={version:9,ready:true,force,ensure,activate,guardMs:TOUCH_CLICK_GUARD_MS};
})();