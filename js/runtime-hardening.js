...
function write(key,value){try{localStorage.setItem(key,value);return true}catch(e){console.warn('Emoji Drops storage write failed:',key,e);return false}}
...
function repair(){if(!storageProbe())return;const raw=readUsers();if(!raw){write('users','[]');return}const cleaned=raw.map(cleanUser).filter(Boolean);const current=localStorage.getItem('currentUser');if(current&&!cleaned.some(u=>u.email===current)){try{localStorage.removeItem('currentUser')}catch{}}if(JSON.stringify(raw)!==JSON.stringify(cleaned))write('users',JSON.stringify(cleaned));write(SCHEMA,'2')}
function migrateReward(){try{const email=localStorage.getItem('currentUser');if(!email)return;const old=localStorage.getItem(`${REWARD_V6}:${email}`),neo=localStorage.getItem(`${REWARD_V7}:${email}`);if(old&&!neo)write(`${REWARD_V7}:${email}`,old)}catch{}}
