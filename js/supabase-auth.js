...
const publicKey=()=>String(cfg.publishableKey||cfg.anonKey||'');
const A=window.EmojiDropsAuth={configured:Boolean(cfg.url&&publicKey()),client:null,userId:null,__v3:true};
...
function headers(token){return {'apikey':cfg.anonKey,'Authorization':`Bearer ${token||publicKey()}`,'Content-Type':'application/json','Accept':'application/json'}}
  ...
  channel:(name)=>{
   ...
   const api={
    ...
    subscribe:async(cb)=>{statusCb=typeof cb==='function'?cb:null;
      ...
      const url='wss://'+host+'/realtime/v1/websocket?apikey='+encodeURIComponent(publicKey())+'&vsn=1.0.0';
      try{
