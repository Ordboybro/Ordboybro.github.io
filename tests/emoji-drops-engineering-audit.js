...
const walk=(dir,ext)=>fs.readdirSync(path.join(ROOT,dir),{withFileTypes:true}).flatMap(e=>e.isDirectory()?walk(path.join(dir,e.name),ext):e.name.endsWith(ext)?[path.join(dir,e.name)]:[]);
...
// Historical version files are only removable when they are proven unused by the production graph.
...
// Detect exact duplicate CSS rule blocks without trying to rewrite cascade semantics.
...
// Static dangerous-code and injection audit.
...
// Numeric safety audit: production code must guard non-finite user/state numbers before committing them.
