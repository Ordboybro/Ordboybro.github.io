...
if(!/const version=335;/.test(app)||!index.includes('js/app-v2.js?v=runtime-335'))throw Error('Runtime owner/cache version drift');
...
for(const [name,file] of requiredOwners){if(!fs.existsSync(file))throw Error(`Missing canonical ${name} owner: ${file}`);if(!app.includes(file))throw Error(`Canonical ${name} owner not loaded by runtime manifest`)}
...
const runtimeManifest=app.match(/const files=\[(.*?)\];/s)?.[1]||'';if((runtimeManifest.match(/emoji-drops-live-final\.js/g)||[]).length!==1)throw Error('Live Drops owner loaded more than once');
