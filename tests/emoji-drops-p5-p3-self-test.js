...
for(const x of ['js/app-v2.js','js/emoji-drops-p5-final-audit.js','js/emoji-drops-p4-final.js','js/emoji-drops-final-product-polish.js','js/emoji-drops-live-final.js']){const s=fs.readFileSync(x,'utf8');if(/prompt\s*\(/.test(s))throw Error('Interactive prompt() remains in canonical UI layer: '+x)}
if(!/No new product controls/.test(p5))throw Error('P5 scope marker missing');
const files=['js/app-v2.js','js/emoji-drops-p5-final-audit.js','js/emoji-drops-p4-final.js','js/emoji-drops-final-product-polish.js','js/emoji-drops-live-final.js'];
