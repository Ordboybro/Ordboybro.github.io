...
for(const x of ['p5-final-audit-1','emoji-drops-p5-final-audit.js?v=p5-final-audit-1'])if(!app.includes(x))throw Error('P5 runtime manifest missing: '+x);
for(const x of ['prefers-reduced-motion','min-width:48px','min-height:48px','PerformanceObserver','aria-hidden','data-ed-rarity'])if(!p5.includes(x))throw Error('P5 contract missing: '+x);
