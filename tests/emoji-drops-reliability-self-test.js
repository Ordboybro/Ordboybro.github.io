...
ctx.window.__emojiDropsFaults={transaction:false,storageWrite:false,storageRead:false};const tx=t.begin('storage-failure');if(!tx)throw Error('Storage-failure setup failed');ctx.window.__emojiDropsFaults.storageWrite=true;s.balance=100;let threw=false;try{localStorage.setItem('emojiDropsStateV3',JSON.stringify(s))}catch{threw=true}if(!threw)throw Error('Storage write fault was not injected');if(s.balance!==250)throw Error('Storage failure did not restore state');
