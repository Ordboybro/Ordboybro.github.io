...
),'g'))||[]).length;if(count!==2)throw new Error('Unbalanced dollar quote: '+tag+' count='+count)}
if(/\\bas\\s+\\$\\s*\\n/.test(schema)||/\\n\\$;/.test(schema))throw new Error('Malformed bare dollar-quote delimiter detected');
if(!schema.toLowerCase().includes('revoke execute on function public.case_cost(text) from public,anon'))throw new Error('case_cost must not be executable by public/anon');
