...
(async()=>{
 ...
 for(const [name,body] of deniedRpcs){ const r=await rpc(name,body); const text=await r.text(); if(r.status===300&&/PGRST203|Could not choose the best candidate function|overloaded functions/i.test(text)) throw Error('Production migration not applied: '+name+' has an ambiguous RPC overload (PGRST203). Apply the canonical Market/Live migration before release.'); const denied=[401,403].includes(r.status)||(r.status===400&&/AUTH_REQUIRED|JWT|authentication/i.test(text))||(r.status===404&&/PGRST202|function .* not found|Could not find the function/i.test(text)); assert.ok(denied,'Unauthenticated RPC unexpectedly allowed: '+name+' -> '+r.status+' '+text.slice(0,240)); }
 ...
 const live=await req('live_drops?select=nickname,item,case_id,item_price,created_at&limit=1');assert.equal(live.status,200,`live_drops public read failed: ${live.status}`);const liveRows=await live.json();assert.ok(Array.isArray(liveRows));for(const row of liveRows)for(const k of Object.keys(row))assert.ok(['nickname','item','case_id','item_price','created_at'].includes(k),`Unexpected live_drops field exposed: ${k}`);
