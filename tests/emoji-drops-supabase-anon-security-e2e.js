...
(async()=>{
 ...
 for(const [name,body] of deniedRpcs){ const r=await rpc(name,body); const text=await r.text(); const denied=[401,403].includes(r.status)||(r.status===300&&/PGRST203|Could not choose the best candidate function/i.test(text))||(r.status===400&&/AUTH_REQUIRED|JWT|authentication/i.test(text))||(r.status===404&&/PGRST202|function .* not found|Could not find the function/i.test(text)); assert.ok(denied,'Unauthenticated RPC unexpectedly allowed: '+name+' -> '+r.status+' '+text.slice(0,240)); }
