...
async function closeActiveModal(page,width,height){
  const modal=page.locator('.ed-modal.show').last();
  if(!(await modal.count()))return;
  const close=modal.locator('.ed-close,[data-close]').first();
...
async function assertCore(page,width,height){
  ...
  if(await openCase.count()){
    ...
    await closeActiveModal(page,width,height);
  }
  await assertNoBlockingModal(page,width,height);
