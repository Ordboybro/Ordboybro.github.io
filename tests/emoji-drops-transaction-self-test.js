...
if(!t||t.version!==10||typeof t.run!=='function'||t.maxHistory!==200||t.journal!=='emojiDropsTxnV4'||t.commitGraceMs!==1200||t.nonBlockingLease!==true||t.faultAware!==true||t.leaseOwnershipVerified!==true||t.webLocksPrimary!==false)throw new Error('Transaction v9 contract missing');
...
const j=JSON.parse(storage.get('emojiDropsTxnV4'));if(j.v!==10||j.status!=='committed'||!j.afterHash||!j.beforeHash)throw new Error('Committed journal metadata missing');
...
(async()=>{let held=false;await t.withLock(()=>new Promise(resolve=>{held=Boolean(storage.get('emojiDropsLockV4'));resolve()}));if(!held)throw new Error('Fallback lock was not held for async transaction');if(storage.has('emojiDropsLockV4'))throw new Error('Fallback lock was not released');console.log('Transaction v9 self-test OK: journal, rollback, hash verification, Web Locks primary contract, lease ownership, stale/lost-lock recovery, async fallback lifetime and fault injection')})().catch(err=>{console.error(err);process.exitCode=1});
