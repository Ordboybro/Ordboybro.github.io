...
if(!t||t.version!==2||t.maxHistory!==200)throw new Error('Transaction layer contract missing');
const s=ctx.window.__emojiDropsCore._s,before=JSON.parse(JSON.stringify(s));
const tx=t.begin('test');
if(!tx||!tx.before||!storage.has('emojiDropsTxnV2'))throw new Error('Journal begin failed');
...
const j=JSON.parse(storage.get('emojiDropsTxnV2'));if(j.status!=='committed')throw new Error('Committed journal missing');
console.log('Transaction self-test OK: journal, rollback, immediate commit verification');
