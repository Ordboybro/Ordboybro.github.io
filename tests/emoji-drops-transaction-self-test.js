...
(async()=>{let held=false;await t.withLock(()=>new Promise(resolve=>{held=Boolean(storage.get('emojiDropsLockV4'));resolve()}));if(!held)throw new Error('Fallback lock was not held for async transaction');if(storage.has('emojiDropsLockV4'))throw new Error('Fallback lock was not released');console.log('Transaction v8 self-test OK: journal, rollback, hash verification, Web Locks primary contract, lease ownership, stale/lost-lock recovery, async fallback lifetime and fault injection')})().catch(err=>{console.error(err);process.exitCode=1});
