...
const ctx={window:{addEventListener:()=>{}},document:{addEventListener:()=>{},querySelector:()=>null},localStorage:{getItem:k=>storage.get(k)??null,setItem:(k,v)=>storage.set(k,String(v)),removeItem:k=>storage.delete(k)},console,setTimeout:(fn)=>fn(),Date,JSON,Math};
...
if(!t||t.version!==3||t.maxHistory!==200||t.journal!=='emojiDropsTxnV3')throw new Error('Transaction v3 contract missing');
const s=ctx.window.__emojiDropsCore._s,before=JSON.parse(JSON.stringify(s));
const tx=t.begin('test');
if(!tx||!tx.before||!storage.has('emojiDropsTxnV3'))throw new Error('Journal begin failed');
...
const j=JSON.parse(storage.get('emojiDropsTxnV3'));if(j.status!=='committed'||!j.afterHash||!j.beforeHash)throw new Error('Committed journal metadata missing');
console.log('Transaction v3 self-test OK: journal, rollback, hash verification, commit metadata');
