...
for(const [key,list] of Object.entries(cases)){
  ...
  const seen=new Set();
  for(const x of list){
    const fp=`${x.emoji}|${x.rarity}|${x.price}`;
    if(seen.has(fp))throw new Error(`${key}: duplicate item ${fp}`);seen.add(fp);
    const value=Number.parseFloat(String(x.price).replace(/[^0-9.\-]/g,''));
    if(!Number.isFinite(value)||value<0)throw new Error(`${key}: invalid item price`);
  }
}
let seed=0x9e3779b9;
...
function openModel(balance,price,item){if(!Number.isFinite(balance)||balance<price)throw new Error('open allowed below case price');return {balance:balance-price,inventory:[item],spent:price}}
function sellModel(balance,inventory,index){if(!Number.isFinite(balance)||index<0||index>=inventory.length)throw new Error('invalid inventory transaction');const item=inventory[index];const price=Number(item.price);if(!Number.isFinite(price)||price<0)throw new Error('invalid item sale value');return {balance:balance+price,inventory:inventory.filter((_,i)=>i!==index)}}
function upgradeModel(source,target,win){if(!source||!target||source===target)throw new Error('invalid upgrade targets');const inv=[source,target];const next=win?[target]:[source];if(next.length!==1)throw new Error('upgrade transaction invariant');return next}
