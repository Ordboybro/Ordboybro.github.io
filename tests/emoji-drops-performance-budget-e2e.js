  ...
  for(let i=0;i<40;i++){
    try{
      await new Promise((resolve,reject)=>{
        const req=require('node:http').get('http://127.0.0.1:4176/',r=>{r.resume();r.statusCode===200?resolve():reject(Error('HTTP '+r.statusCode))});
        req.on('error',reject);
      });
      return;
  ...
  try{
    ...
    try{
      ...
      await page.addInitScript(()=>{
        ...
        window.__edLifecycle={adds:0,removes:0,activeObservers:0,observerCreates:0,observerDisconnects:0,timerAdds:0,timerClears:0,intervalAdds:0,intervalClears:0,audioCreates:0};
        const A=EventTarget.prototype.addEventListener,R=EventTarget.prototype.removeEventListener;
        EventTarget.prototype.addEventListener=function(...a){window.__edLifecycle.adds++;return A.apply(this,a)};
        EventTarget.prototype.removeEventListener=function(...a){window.__edLifecycle.removes++;return R.apply(this,a)};
        const MO=window.MutationObserver;
        if(MO){
          window.MutationObserver=function(...a){
            window.__edLifecycle.observerCreates++;window.__edLifecycle.activeObservers++;
            const o=new MO(...a),d=o.disconnect.bind(o);
            o.disconnect=()=>{if(o.__edDisconnected)return; o.__edDisconnected=true; window.__edLifecycle.activeObservers=Math.max(0,window.__edLifecycle.activeObservers-1);window.__edLifecycle.observerDisconnects++;return d()};
            return o;
          };
        ...
        window.setTimeout=function(...a){window.__edLifecycle.timerAdds++;return st(...a)};
        window.clearTimeout=function(...a){window.__edLifecycle.timerClears++;return ct(...a)};
        window.setInterval=function(...a){window.__edLifecycle.intervalAdds++;return si(...a)};
        window.clearInterval=function(...a){window.__edLifecycle.intervalClears++;return ci(...a)};
        if(window.AudioContext){const AC=window.AudioContext;window.AudioContext=function(...a){window.__edLifecycle.audioCreates++;return new AC(...a)}}
        if(window.PerformanceObserver){try{const o=new PerformanceObserver(list=>{for(const x of list.getEntries())window.__edLongTasks.push(x.duration)});o.observe({entryTypes:['longtask']});window.__edLongTaskObserver=true}catch{window.__edLongTaskObserver=false}}
      });
      ...
      const initial=await page.evaluate(()=>{
        ...
        return {load:nav?.loadEventEnd||0,resources:performance.getEntriesByType('resource').length,nodes:document.getElementsByTagName('*').length,life:{...window.__edLifecycle}};
      });
