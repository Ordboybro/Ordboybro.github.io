...
if(!/version[:=]5/.test(tx)||!/beforeHash/.test(tx)||!/afterHash/.test(tx)||!/COMMIT_GRACE_MS/.test(tx))throw Error('Transaction v5 verification contract missing');
if(/setInterval\s*\(/.test(product))throw Error('Product layer must not use polling intervals');
console.log('Product self-test OK: semantic Emoji Coin, catalog/inventory tools, progression, achievements, persistence/performance contracts, transaction v5 compatibility');
