...
if(!/emoji-drops-transaction-layer\.js\?v=txn-11/.test(loader))throw Error('Product loader transaction marker missing: txn-8/txn-9');
...
if(!/version[:=]10/.test(tx)||!/beforeHash/.test(tx)||!/afterHash/.test(tx)||!/COMMIT_GRACE_MS/.test(tx)||!/nonBlockingLease/.test(tx)||!/faultAware/.test(tx))throw Error('Transaction v10 verification contract missing');
