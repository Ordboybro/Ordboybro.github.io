...
for(const marker of ['emoji-drops-product-layer.js?v=product-1','product-layer','emoji-drops-transaction-layer.js?v=txn-7'])if(!loader.includes(marker))throw Error(`Product loader marker missing: ${marker}`);
...
if(!/version[:=]7/.test(tx)||!/beforeHash/.test(tx)||!/afterHash/.test(tx)||!/COMMIT_GRACE_MS/.test(tx)||!/nonBlockingLease/.test(tx)||!/faultAware/.test(tx))throw Error('Transaction v7 verification contract missing');
if(/setInterval\s*\(/.test(product))throw Error('Product layer must not use polling intervals');
console.log('Product self-test OK: semantic Emoji Coin, catalog/inventory tools, progression, achievements, persistence/performance contracts, transaction v7 compatibility');
