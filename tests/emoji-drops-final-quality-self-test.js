...
const cases=ctx.cases||w.cases||{};const prices=ctx.casePrices||w.casePrices||{};
...
const s=ctx.window.__emojiDropsCore._s,original=JSON.stringify(s),a=t.begin('quality');if(!a)throw Error('Transaction begin failed');s.balance=999;s.inventory.push({id:'q',emoji:'😀',rarity:'common',price:'10₽'});if(!t.rollback(a)||JSON.stringify(s)!==original)throw Error('Rollback invariant failed');
