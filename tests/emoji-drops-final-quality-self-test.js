...
vm.runInContext(tx,ctx,{timeout:2000});const t=ctx.window.__emojiDropsTransactions;if(t?.version!==8||t?.journal!=='emojiDropsTxnV4'||t?.commitGraceMs!==1200||t?.nonBlockingLease!==true||t?.faultAware!==true)throw Error('Transaction v8 contract failed');
...
console.log('Final quality self-test OK: dataset, rarities, hardening, v8 transaction rollback, long-session balance invariant');
