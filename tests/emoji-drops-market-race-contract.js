...
if(!/perform 1 from public\.profiles where id in \(uid,seller_id\) order by id for update;[\s\S]*select \* into l from public\.market_listings where id=p_listing_id for update/i.test(buy))throw new Error('Buy must lock ordered profiles before listing');
if(!/select seller_id into seller_id from public\.market_listings[\s\S]*?perform 1 from public\.profiles where id=uid for update;[\s\S]*select \* into l from public\.market_listings where id=p_listing_id for update/i.test(cancel))throw new Error('Cancel must lock profile before listing');
