...
for(const v of ['schema-1','hardening-9','core-7','txn-7','guards-8','ui-3','final-2','release-1','product-1','product-plus-1','action-6','quality-final-1','p3-2','p4-2'])if(!loader.includes(v))throw Error(`Cache version missing: ${v}`);
...
for(const x of ['Emoji Drops action resilience v6','emojiDropsActionResilienceV6','MutationObserver','normalize','data-do-open','data-sell-all','data-upgrade','data-daily','setAttribute','visibleModal','closeModal','Escape','focusables'])if(!action.includes(x))throw Error(`Action resilience contract missing: ${x}`);
...
console.log('Static audit OK: 14-layer runtime, schema migration, transaction v7 recovery/fault injection, action resilience v6, P3 interaction polish, P4 engineering boundary/touch target guarantee, accessibility/performance/privacy diagnostics');
