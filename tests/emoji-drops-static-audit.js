...
for(const v of ['schema-1','hardening-9','core-7','txn-7','guards-8','ui-3','final-2','release-1','product-1','product-plus-1','action-7','quality-final-1','p3-3','p4-1'])if(!loader.includes(v))throw Error(`Cache version missing: ${v}`);
...
for(const x of ['reduced-motion','ed-p3-skeleton','Sound','vibrate','touchstart','ed-modal','ed-live','tabindex','pan-x'])if(!p3.includes(x))throw Error(`P3 contract missing: ${x}`);
