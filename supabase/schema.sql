...
select case lower(trim(p_case_id)) when 'smile' then 100 when 'moves' then 80 when 'nature' then 60 when 'food' then 40 when 'animals' then 20 when 'transport' then 10 when 'sport' then 250 when 'games' then 500 else null end $casecost$;
...
create or replace function public.market_snapshot() returns table(
  ...
  select ml.id,(ml.item->>'id'),p.nickname,(ml.item->>'emoji'),(ml.item->>'rarity'),coalesce(ml.item->>'case_id',''),coalesce((ml.item->>'price')::numeric,ml.listing_price),ml.listing_price,ml.status,ml.created_at,(auth.uid() is not null and auth.uid()=ml.seller_id)
